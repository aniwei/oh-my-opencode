...
// 计算单次请求费用
// 费率单位: 美元 / 百万 token
export function calculateCost(model: Model, usage: Usage): CostBreakdown {
  const { cost } = model
...
export class CostTracker {
  private entries: Array<{ model: string; usage: Usage; cost: CostBreakdown }> = []

  // 记录一次请求
  record(model: Model, usage: Usage): CostBreakdown {
