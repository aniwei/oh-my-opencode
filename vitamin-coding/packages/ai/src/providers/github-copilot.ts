...
const log = createLogger('ai:github-copilot')

// Copilot 默认端点
const COPILOT_DEFAULT_BASE_URL = 'https://api.githubcopilot.com'

// Copilot 集成标识
const COPILOT_INTEGRATION_ID = 'vitamin-coding-agent'

// Copilot 编辑器版本标识
const COPILOT_EDITOR_VERSION = 'vitamin-coding/1.0.0'
...
function buildCopilotHeaders(apiKey: string): Record<string, string> {
  return {
    ...
    'editor-plugin-version': 'vitamin-coding/1.0.0',
...
function buildFinalMessage(state: CopilotStreamState): AssistantMessage {
...
export function createCopilotProvider(): ProviderAdapter {
  return {
    id: 'github-copilot',
    ...
    async *stream(
      ...
      const baseUrl = model.baseUrl || COPILOT_DEFAULT_BASE_URL
      const url = `${baseUrl}/chat/completions`
      const body = buildRequestBody(model, context)

      log.debug({ model: model.id }, 'GitHub Copilot 流式请求')
    ...
    async healthCheck(apiKey: string): Promise<boolean> {
      try {
        const response = await fetch(`${COPILOT_DEFAULT_BASE_URL}/models`, {
          headers: buildCopilotHeaders(apiKey),
        })
        return response.ok
      } catch {
